...
const route = useRoute(); // 현재 경로에서 파라미터를 가져오기 위해 사용
...
const addFeed = async (feedInfo) => {
  try {
    ...
    if (response.data.success) {
      ...
      router.push(`/garden-diary/${username.value}`); // 유저 이름을 포함한 다이어리 경로로 이동
    } else {
...
// 다이어리 목록으로 돌아가기 함수
const goBackToDiary = () => {
  router.push(`/garden-diary/${username.value}`); // 유저 이름을 포함한 다이어리 목록 경로로 이동
};
</script>
